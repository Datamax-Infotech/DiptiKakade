...
db_project_mgmt();
function getCountOfProjectForSidebarMenu(): int
{
	$project_count_sql = db_query("SELECT project_id FROM project_master where  project_owner = '" . $_COOKIE["b2b_id"] . "' and archive_status=0");
	$project_count = tep_db_num_rows($project_count_sql);
	return $project_count;
}
function getMeetingProjectDataAfterAction(int $meeting_id): array
{
	$project_sql = db_query("SELECT project_id,project_name,project_owner FROM project_master where find_in_set($meeting_id,meeting_ids) and archive_status=0 ORDER BY project_id DESC");
	$result = array();
...
function getAllProjectDataAfterAction(): array
{
	db();
	$select_level = db_query("SELECT level from loop_employees where b2b_id =" . $_COOKIE['b2b_id']);
	...
	db_project_mgmt();
	//$sql_main = db_query("SELECT id, meeting_name FROM meeting_master where status = 1 union SELECT 0, 'Personal' ORDER BY (meeting_name <> 'Personal') ASC,meeting_name");
	...
	union SELECT 0, 'Personal' ORDER BY (meeting_name <> 'Personal') ASC,meeting_name ");

	$result = array();
	while ($main_row = array_shift($sql_main)) {
		$meeting_id = $main_row['id'];
		$count_sql = db_query("SELECT project_id FROM project_master where find_in_set($meeting_id,meeting_ids) AND project_owner = '" . $_COOKIE["b2b_id"] . "' and archive_status=0 ORDER BY project_id DESC");
		$count = tep_db_num_rows($count_sql);
		if ($count > 0) {
			$sql1 = db_query("SELECT project_id,project_name, project_description,project_owner,project_status_id, project_deadline FROM project_master where find_in_set($meeting_id,meeting_ids) AND project_owner = '" . $_COOKIE["b2b_id"] . "' and archive_status=0 ORDER BY project_id DESC LIMIT 10");
			$data = array();
...
function getAllProjectDataAfterWorkSpaceAction(int $meeting_id, string $filter_str): array
{
	$sql1 = db_query("SELECT project_id,project_name, project_description,project_status_id, project_deadline,project_owner FROM project_master where find_in_set($meeting_id,meeting_ids) and archive_status=0 $filter_str ORDER BY project_id DESC LIMIT 10");
	$data = array();
	while ($r = array_shift($sql1)) {
		$status_data = get_status_date_color_info($r['project_status_id'], $r['project_deadline']);
		db();
...
if (isset($_GET['edit_project']) && $_GET['edit_project'] == 1) {
	...
	$sql1 = db_query("SELECT * FROM project_master where project_id = $project_id ");

	$data = [];
	while ($r = array_shift($sql1)) {
		$data = $r;
		$sql2 = db_query("SELECT checked, milestone,milestone_date FROM project_milestones where project_id = $project_id ORDER BY milestone_date");
		$data_milestone = [];
...
if (isset($_GET['get_project_status']) && $_GET['get_project_status'] == 1) {
	$query = db_query("SELECT id,status FROM project_status order by id");
	$data = [];
...
if (isset($_POST["project_action"]) && $_POST["project_action"] != "") {
	...
	if (!empty($_FILES['uploadscanrep'])) {
		...
		if ($tmppos_1 != false) {
			...
			$lastPipePosition = strrpos($scan_rep_name, '|');
			if ($lastPipePosition !== false) {
				$scan_rep_name .= "|" . substr($scan_rep_name, 0, $lastPipePosition);
			}
			/*if ($scan_rep_name != "") {
	...
	if ($_POST["project_action"] == "ADD" || $_POST['project_action'] == "ADD_FROM_MEET" ||  $_POST['project_action'] == "ADD_FROM_MEET_START_MEETING" || $_POST["project_action"] == "ADD_FROM_WORKSPACE") {
		...
		if (isset($_POST['milestone_title'])) {
		...
		if ($_POST['project_action'] == "ADD_FROM_MEET_START_MEETING") {
			...
			db_query("UPDATE meeting_live_updates set project_flg=1 where meeting_timer_id='" . $_POST['meeting_timer_id'] . "' && attendee_id!='" . $_COOKIE['b2b_id'] . "'");
		}
...
if (isset($_REQUEST['load_type']) && $_REQUEST['load_type'] == "project") {
	...
	$sql1 = db_query("SELECT project_id,project_name, project_description,project_owner,project_status_id, project_deadline FROM project_master where find_in_set($project_meeting_id,meeting_ids) AND project_owner = '" . $owner_id . "' and archive_status=0 ORDER BY project_id DESC LIMIT 10 OFFSET $loaded_data");
	while ($r = array_shift($sql1)) {
		$status_data = get_status_date_color_info($r['project_status_id'], $r['project_deadline']);
		$status_name = $status_data['status_name'];
		$status_class = $status_data['status_class'];
...
if (isset($_GET['delete_project']) && $_GET['delete_project'] == 1) {
	...
	if (isset($_GET['delete_from_workspace']) && $_GET['delete_from_workspace'] == 1) {
		...
		db_query($sql_delete_project);
		$select_already_esiting_id = db_query("SELECT meeting_ids from project_master where project_id=$project_id");
...
if (isset($_POST["meeting_project_list"]) && $_POST["meeting_project_list"] == 1) {
	db_project_mgmt();
	$meeting_id = $_POST['meeting_id'];
	$project_sql_main = db_query("SELECT project_id from project_master where archive_status=0 and find_in_set('" . $meeting_id . "',`meeting_ids`)");
	...
	$project_sql = db_query("SELECT project_id,project_name,project_owner FROM project_master $filter_out_already_added_pro ORDER BY project_id DESC");
	$result = array();
	while ($r = array_shift($project_sql)) {
		db();
		$empDetails_qry = db_query("SELECT Headshot, name,initials from loop_employees where b2b_id='" . $r['project_owner'] . "'");
		$empDetails_arr = array_shift($empDetails_qry);
		$empDetails = getOwerHeadshotForMeeting($empDetails_arr['Headshot'], $empDetails_arr['initials']);
		$result[] = array('project_name' => $r["project_name"], 'project_id' => $r['project_id'], 'emp_img' => $empDetails['emp_img'], 'emp_txt' => $empDetails['emp_txt'], 'project_owner' => $empDetails_arr['name']);
	}
	echo json_encode($result);
}
...
if (isset($_POST["add_existing_project_meeting"]) && $_POST["add_existing_project_meeting"] != "") {
	...
	foreach ($_POST['project_id'] as $project_id) {
		$select_already_esiting_id = db_query("SELECT meeting_ids from project_master where archive_status=0 and project_id=$project_id");
		$meet_ids = array_shift($select_already_esiting_id)['meeting_ids'];
...
if (isset($_POST["meeting_project_action"]) && $_POST["meeting_project_action"] == "meeting_project_edit") {
	...
	$project_old_data_sql = db_query("SELECT project_id,project_name,project_deadline,project_owner,project_description FROM project_master where project_id=$project_id");
	...
	if ($_POST["owner_id"] != $project_old_data['project_owner']) {
		db();
		$select_owner_name = db_query("SELECT name from loop_employees where b2b_id=" . $_POST["owner_id"]);
	...
	db_query("UPDATE meeting_live_updates set project_flg=1 where meeting_timer_id='" . $_POST['meeting_timer_id'] . "' && attendee_id!='" . $_COOKIE['b2b_id'] . "'");
	$sql1 = db_query("SELECT project_id,project_name,project_status_id,project_deadline,project_owner FROM project_master  where project_id = $project_id and archive_status=0");
	...
	while ($r = array_shift($sql1)) {
		...
		db_project_mgmt();
		$sql2 = db_query("SELECT checked, milestone,milestone_date FROM project_milestones where project_id = $project_id ");
		$data_milestone = [];
		while ($rowsel_getdata = array_shift($sql2)) {
			$data_milestone[] = array('checked' => $rowsel_getdata["checked"], 'milestone' => $rowsel_getdata['milestone'], 'milestone_date' => $rowsel_getdata['milestone_date']);
		}
...
if (isset($_GET['update_meeting_project_status']) && $_GET['update_meeting_project_status'] == 1) {
	...
	db_query($insql);
	$sql = db_query("SELECT `project_deadline` FROM `project_master` WHERE `project_id` = $project_id");
	...
	db_query("UPDATE meeting_live_updates set project_flg=1 where meeting_timer_id='" . $_GET['meeting_timer_id'] . "' && attendee_id!='" . $_COOKIE['b2b_id'] . "'");
	echo json_encode($res);
}
...
if (isset($_GET['project_detail_to_add_task_issue']) && $_GET['project_detail_to_add_task_issue'] == 1) {
	...
	$sql1 = db_query("SELECT project_deadline,project_id,project_owner,project_name,project_enter_on,project_status.status as p_status,project_status_id FROM project_master 
		JOIN project_status ON project_status.id=project_master.project_status_id where project_id = $project_id ");
	$data = array_shift($sql1);
	db();
	$empDetails_qry = db_query("SELECT name from loop_employees where b2b_id='" . $data['project_owner'] . "'");
...
if (isset($_GET['get_emp_department']) && $_GET['get_emp_department'] == 1) {
	db();
	$select_dept = db_query("SELECT dept_id FROM loop_employees where b2b_id=" . $_GET['current_emp']);
