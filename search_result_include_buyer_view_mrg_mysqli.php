...
function delivery_financials_reload(compid, rec_id, warehouse_id, rec_type) {
    ...
    xmlhttp.open("POST", "loop_shipbubble_delivery_financials_update.php?ID=" + compid + "&rec_id=" + rec_id +
...
function TroublewithShipper_eml_load(compid, rec_id, warehouse_id, rec_type) {
    ...
    xmlhttp.open("POST", "sendemail_trouble_with_shipper.php?compid=" + compid + "&rec_id=" + rec_id +
        "&warehouse_id=" + warehouse_id + "&rec_type=" + rec_type, true);
    xmlhttp.send();
}
...
function readytoship_email(compid, rec_id, warehouse_id, rec_type) {
    ...
    xmlhttp.open("POST", "sendemail_readytoship.php?compid=" + compid + "&rec_id=" + rec_id + "&warehouse_id=" +
        warehouse_id + "&rec_type=" + rec_type + "&txtnotesforfreight=" + txtnotesforfreight, true);
    xmlhttp.send();
}
...
function readytoship_email_cpu(compid, rec_id, warehouse_id, rec_type) {
    ...
    xmlhttp.open("POST", "sendemail_readytoship_cpu.php?compid=" + compid + "&rec_id=" + rec_id + "&warehouse_id=" +
        warehouse_id + "&rec_type=" + rec_type + "&txtnotesforfreight=" + txtnotesforfreight, true);
    xmlhttp.send();
}
</script>
...
<Font Face='arial' size='2'><br><br>
    ...
    <span id="viewdiv<?php echo $_REQUEST["rec_id"]; ?>">
        ...
        <iframe frameborder="0" onload="iframePoLoaded()" scrolling="auto" id="iframe_po_display"
            name="iframe_po_display"
            src="loop_sales_iframe_po_display.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
        ...
        $payment_type = "";
        $po_delivery_dt = "";
        $sql = "SELECT `trouble_with_shipper_ignore`, po_payment_method, trouble_with_shipper_ignore_dt, ops_delivery_date, po_delivery_dt, trouble_with_shipper_ignore_user FROM loop_transaction_buyer WHERE id = " . $_REQUEST["rec_id"];
        db();
        ...
        db();
        $getdata = db_query("Select trouble_with_shipper_email_sendon, trouble_with_shipper_email_sendby From loop_transaction_buyer_poeml where trans_rec_id = " . $_REQUEST["rec_id"] . " and trouble_with_shipper_email_sendon <> '' limit 1");
        ...
        ?>
        <br>
        <iframe frameborder="0" onload="iframeLoaded_cc_details()" width="600px" height="150" scrolling="auto"
            id="iframe_cc_details"
            src="loop_orderbubble_cc_details.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&rec_id=<?php echo $_REQUEST["rec_id"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
        </iframe>
        ...
        <table cellSpacing="1" cellPadding="1" border="0" style="width: 400px; font-size:10;" id="table14">
            ...
            <?php
                ...
                $sql = "SELECT * FROM quote_to_item WHERE quote_id= '" . isset($quote_number) . "' ORDER BY sort_order ASC";
                ...
                while ($bx = array_shift($boxsql)) {
                    ...
                    //echo $c;
                    $d = "select * from boxes where ID = '" . $bx["item_id"] . "'";
                    db_b2b();
                    $itemSql = db_query($d);
                    $item = array_shift($itemSql);

                    $loop_id_chk = "";
                    $sql_forbox = "SELECT id FROM loop_boxes WHERE b2b_id = '" . $item["inventoryID"] . "' ";
                    db();
                    $box_data = db_query($sql_forbox);
            ...
            <tr bgColor="#e4e4e4">
                        ...
                        <font size="1">
                            ...
                            <?php
                                    ...
                                    if ($item['inventoryID'] == 0) {
                                        ...
                                        $z = "select * from inventory Where ID = '" . $item['inventoryID'] . "'";
                                        //echo $z;
                                        db_b2b();
                                        $boxSql = db_query($z);
                                        $objInv = array_shift($boxSql);

                                        $q1 = "SELECT * FROM vendors where id = " . $objInv["vendor"];
                                        db_b2b();
                                        $query = db_query($q1);
                                        $vender_nm = "";
                                    ...
                                    if (isset($min_fob) == 0) {
                                        ...
                                        $min_fob_val = isset($min_fob);
                                    }

                                    //echo "p1: " . $item["ulinePrice"] . " p2:" . $item["salePrice"] . " p3: " . $item["shipfinal"] . " p4:" . $item["quantity"] . "<br>";
                                    ?>
                <td><?php echo $vender_nm; ?></td>
                <td><?php echo isset($b2b_location_city) . " " . isset($b2b_location_st) . " " .    isset($b2b_location_zip); ?>
                </td>
                ...
                <td>
                    ...
                    <?php
                            ...
                            if ($item["salePrice"] >= isset($min_delv_cost)) {


                            ?>

                    <font face="Arial, Helvetica, sans-serif" size="1" color='green'>
                        $<?php echo isset($min_delv_cost); ?>
                    </font>
                    <?php } else { ?>
                    <font face="Arial, Helvetica, sans-serif" size="1" color='red'>$<?php echo isset($min_delv_cost); ?>
                    </font>
                    <?php   }
                            ?>
                </td>
        ...
        <iframe frameborder="0" onload="iframeLoaded_sales_order()" scrolling="auto" id="iframe_sales_order_display"
            src="loop_sales_iframe_sales_order.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&rec_id=<?php echo $_REQUEST["rec_id"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
        ...
        function iframeLoaded_sales_order() {
            ...
            //alert(ifrmaeobj.contentWindow.document.body.scrollHeight);
            //alert(ifrmaeobj.contentWindow.document.body.clientHeight);
            //alert(ifrmaeobj.contentWindow.document.body.offsetHeight);
            var objheight = ifrmaeobj.contentWindow.document.body.offsetHeight;
            objheight = objheight + 50;
            ifrmaeobj.style.height = objheight + 'px';
        ...
        <br>
        <iframe frameborder="0" width="600px" height="100" onload="iframeLoaded()" scrolling="auto"
            id="iframe_virtual_display"
            src="loop_sales_iframe_virtual_display.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&rec_id=<?php echo $_REQUEST["rec_id"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
        ...
        <iframe frameborder="0" scrolling="auto" id="iframe_supplier_ship_date"
            src="loop_orderbubble_shipper_so_po.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&rec_id=<?php echo $_REQUEST["rec_id"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
        </iframe>
        ...
        <iframe frameborder="0" onload="iframeLoaded_payment_or_terms()" scrolling="auto" id="iframe_payment_or_terms"
            src="loop_orderbubble_payment_or_terms.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&rec_id=<?php echo $_REQUEST["rec_id"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
        ...
        <br>
        <iframe frameborder="0" onload="iframeLoaded_add_correct()" scrolling="auto" id="iframe_add_correct"
            src="loop_orderbubble_add_confirm.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&rec_id=<?php echo $_REQUEST["rec_id"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
        ...
        <div id="div_iframe_delivery_financials">
            <iframe frameborder="0" onload="iframeLoaded_delivery_financials()" scrolling="auto"
                id="iframe_delivery_financials"
        ...
        <iframe frameborder="0" width="600px" height="200" scrolling="auto" id="iframe_sent_to_supplier"
            src="loop_orderbubble_sent_to_supplier.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&rec_id=<?php echo $_REQUEST["rec_id"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
        </iframe>

        <br>

        <iframe frameborder="0" onload="iframeLoaded_supplier_shipdate()" scrolling="auto"
            id="iframe_supplier_ship_date"
            src="loop_orderbubble_supplier_ship_date.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&rec_id=<?php echo $_REQUEST["rec_id"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
        </iframe>
        <script>
                ...
                <table cellSpacing="1" cellPadding="1" border="0" style="width: 444px">
                    ...
                    <tr bgColor="#e4e4e4">
                        <td align="center" height="13" style="width: 235px" class="style1">
                            ...
                            <input type="button" value="Send Email" id="btnTroublewithShipper_eml"
                                name="btnTroublewithShipper_eml"
                                onclick="TroublewithShipper_eml_load(<?php echo $_REQUEST["ID"]; ?>, <?php echo $_REQUEST["rec_id"]; ?>, <?php echo $_REQUEST["warehouse_id"]; ?>, '<?php echo $_REQUEST["rec_type"]; ?>' );">
                            <?php } else { ?>
                            <input type="button" value="Re-Send Email" id="btnTroublewithShipper_eml"
                                name="btnTroublewithShipper_eml"
                                onclick="TroublewithShipper_eml_load(<?php echo $_REQUEST["ID"]; ?>, <?php echo $_REQUEST["rec_id"]; ?>, <?php echo $_REQUEST["warehouse_id"]; ?>, '<?php echo $_REQUEST["rec_type"]; ?>' );">
                            <font size="1">Email sent by <?php echo $trouble_with_shipper_eml_sendby; ?> on
                                <?php echo date("m/d/Y H:i:s", strtotime($trouble_with_shipper_eml_sendon)) . " CT"; ?>
                            </font>
                            <?php } ?>

                            &nbsp;&nbsp;
                            <?php if ($trouble_with_shipper_ignore == 0) { ?>
                            <input id="btnTroublewithShipper_eml_ignore" type="button" value="Ignore this Step"
                                onclick="po_ignore('Shipper_eml_ignore', <?php echo $_REQUEST["ID"]; ?>,<?php echo $_REQUEST["rec_id"]; ?>,<?php echo $_REQUEST["warehouse_id"]; ?>,'<?php echo $_REQUEST["rec_type"]; ?>');">
                            <?php } else { ?>
                            <font size="1">This Step in ignore by <?php echo $trouble_with_shipper_ignore_user; ?> on
                                <?php echo date("m/d/Y H:i:s", strtotime($trouble_with_shipper_ignore_dt)) . " CT"; ?>
                            </font>
                            <?php } ?>
                        </td>
                    </tr>
        ...
        <span id="viewdiv_r<?php echo $_REQUEST["rec_id"]; ?>">
            ...
            if ($prepay == "Yes") {
            ?>
            <iframe frameborder="0" width="600px" height="100" scrolling="auto" id="iframe_orderbubble_prepay"
                src="loop_orderbubble_prepay.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&rec_id=<?php echo $_REQUEST["rec_id"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
            </iframe>
            <?php } ?>

            <iframe frameborder="0" width="600px" height="200" scrolling="auto" id="iframe_good_to_ship"
                src="loop_orderbubble_good_to_ship.php?rec_id=<?php echo $_REQUEST['rec_id']; ?>&ID=<?php echo $_REQUEST["ID"]; ?>&rec_id=<?php echo $_REQUEST["rec_id"]; ?>&warehouse_id=<?php echo $_REQUEST["warehouse_id"]; ?>&rec_type='<?php echo $_REQUEST["rec_type"]; ?>'">
