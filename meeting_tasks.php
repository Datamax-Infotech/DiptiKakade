<?php session_start();
	...
	<div id="content-wrapper" class="d-flex flex-column">
        ...
        <div class="container-fluid  mt-0" >
            <div class="row justify-content-center mt-4">
                ...
                <?php 
                db_project_mgmt();
                db_query("UPDATE meeting_live_updates set task_flg=0 where meeting_timer_id='".$_COOKIE['meeting_timer_id']."' && attendee_id=".$_COOKIE['b2b_id']);
                $task_order_str_qry = db_query("SELECT task_order from meeting_live_updates where meeting_timer_id='".$_COOKIE['meeting_timer_id']."' && attendee_id=".$_COOKIE['b2b_id']);
				...
				FROM task_master  where  archive_status=0 and task_meeting=$meeting_id $order_str, id DESC");
                $no_data_div = "d-none";
                            ...
                            <div class="card">
                                <div class="card-body py-3">
                                    ...
                                    <select class="form-control form-control-sm w-25"  id="sort_task">
                                        <option value="0" <?php echo $sort_task == 0 ? "selected ": ""; ?>>Sort</option>
                                        <option value="1" <?php echo $sort_task == 1 ? "selected ": ""; ?>>By Owner</option>
                                        <option value="2" <?php echo $sort_task == 2 ? "selected ": ""; ?>>Incomplete</option>
                                        <option value="3" <?php echo $sort_task == 3 ? "selected ": ""; ?>>By Due Date (Desc)</option>
                                        <option value="4" <?php echo $sort_task == 4 ? "selected ": ""; ?>>By Due Date (Asc)</option>
                                        <option value="5" <?php echo $sort_task == 5 ? "selected ": ""; ?>>By Date Created (Oldest)</option>
                                        <option value="6" <?php echo $sort_task == 6 ? "selected ": ""; ?>>By Date Created (Newest)</option>
                                    </select>
                                    </div>  
                                    <table id="meetingTODO" class="meetingTODOIssue table table_vetical_align_middle table-sm border-0 mb-0">
                                        ...
                                        <?php while($r = array_shift($task_sql)){
                                            ...
                                            <tr id="meeting_task_div_<?php echo $r['id'];?>">
                                                ...
                                                <td class="w-100"><a type="button" <?php echo $r['task_status']==2 || $r['task_status']==1 ? "style='text-decoration:line-through'": "";?> href="javascript:void(0);" class="showToDoDetails task_title" task_id="<?php echo $r['id']; ?>"><?php echo $r['task_title']; ?></a></td>
                            ...
                            <form id="meeting_task_save" onsubmit="return false">
                                <div class="row">
                                    ...
                                    <div class="reports_to_div search_existing_user col-md-12 meeting_assigned_todo">
                                            ...
                                            <?php
                                            db();
                                            $result = db_query("SELECT Headshot,b2b_id, name,initials FROM loop_employees where status='Active' order by id" );
											...
											<?php echo getMeetingEmployeeWithImgForMeetingForms("meeting_task_assigned_to_id","assignto", $meeting_id);?>
                                        </div>
                                    </div>
   ...
   <?php
    ...
    $(document).ready(function() { 
		...
		var task_id=$(this).attr('task_id');
		var data={task_status,task_id, update_meeting_task_status:1,meeting_id:"<?php echo $meeting_id;?>",meeting_timer_id:"<?php echo $meeting_timer_id;?>",};
		var result="";
		$.ajax({
			url:'task_action.php',
			type:'get',
    ...
    $(document).on('change','#sort_task',function(){
        ...
        $.ajax({
			url:'task_action.php',
			type:'get',
			data:{sort_order,sort_meeting_task:1,meeting_id:"<?php echo $meeting_id;?>",meeting_timer_id:"<?php echo $meeting_timer_id;?>",},
			datatype:'json',
			async: false, 
			success:function(response){
                displayTaskDataAfterMeetingAction(JSON.parse(response));
