...
//         }
//         ///delta?$orderby=receivedDateTime+desc
//         $messageList = json_decode($this->sendGetRequest($this->baseURL . 'users/' . $mailbox . '/mailFolders/Inbox/Messages?$top=400&$orderby=createdDateTime+asc'));
...
//             $attachments = (json_decode($this->sendGetRequest($this->baseURL . 'users/' . $mailbox . '/messages/' . $mailItem->id . '/attachments')));
...
//     function getMessages_morerec($mailbox)
//     {
//         if (!$this->Token) {
//             throw new Exception('No token defined');
//         }
//         ///delta?$orderby=receivedDateTime+desc
//         $messageList = json_decode($this->sendGetRequest($this->baseURL . 'users/' . $mailbox . '/mailFolders/Inbox/Messages?$top=500&$orderby=createdDateTime+asc'));
//         if (isset($messageList->error)) {
//             throw new Exception($messageList->error->code . ' ' . $messageList->error->message);
//         }
//         $messageArray = array();
...
//     function getMessages_folder($mailbox, $folderid)
//     {
//         if (!$this->Token) {
//             throw new Exception('No token defined');
//         }
//         ///delta?$orderby=receivedDateTime+desc
//         $messageList = json_decode($this->sendGetRequest($this->baseURL . 'users/' . $mailbox . '/mailFolders/' . $folderid . '/Messages?$top=400&$orderby=createdDateTime+asc'));
...
//     function moveEmail($mailbox, $id)
//     {
//         $this->sendPostRequest($this->baseURL . 'users/' . $mailbox . '/messages/' . $id . '/move', '{ "destinationId": "drafts" }', array('Content-type: application/json'));
//     }

//     function moveEmail2($mailbox, $id)
//     {
//         $this->sendPostRequest($this->baseURL . 'users/' . $mailbox . '/messages/' . $id . '/move', '{ "destinationId": "scheduled" }', array('Content-type: application/json'));
//     }

//     function moveEmail3($mailbox, $id)
//     {
//         $this->sendPostRequest($this->baseURL . 'users/' . $mailbox . '/messages/' . $id . '/move', '{ "destinationId": "searchfolders" }', array('Content-type: application/json'));
//     }

//     function moveEmail4($mailbox, $id)
//     {
//         $this->sendPostRequest($this->baseURL . 'users/' . $mailbox . '/messages/' . $id . '/move', '{ "destinationId": "clutter" }', array('Content-type: application/json'));
//     }
...
class graphMailer
{
    ...
    function __construct(string $sTenantID, string $sClientID, string $sClientSecret)
    {
        $this->tenantID = $sTenantID;
        $this->clientID = $sClientID;
        $this->clientSecret = $sClientSecret;
    ...
    function getMessages(string $mailbox): array
    {
        if (!$this->Token) {
            throw new Exception('No token defined');
        }
    ...
    function getMessages_morerec(string $mailbox): array
    {
        if (!$this->Token) {
            throw new Exception('No token defined');
        }
    ...
    function getMessages_folder(string $mailbox, string $folderid): array
    {
        if (!$this->Token) {
            throw new Exception('No token defined');
        }
    ...
    function deleteEmail(string $mailbox, string $id, bool $moveToDeletedItems = true): void
    {
        switch ($moveToDeletedItems) {
            case true:
                $this->sendPostRequest($this->baseURL . 'users/' . $mailbox . '/messages/' . $id . '/move', '{ "destinationId": "deleteditems" }', array('Content-type: application/json'));
    ...
    function moveEmail(string $mailbox, string $id): void
    {
        $this->sendPostRequest($this->baseURL . 'users/' . $mailbox . '/messages/' . $id . '/move', '{ "destinationId": "drafts" }', array('Content-type: application/json'));
    }

    function moveEmail2(string $mailbox, string $id): void
    {
        $this->sendPostRequest($this->baseURL . 'users/' . $mailbox . '/messages/' . $id . '/move', '{ "destinationId": "scheduled" }', array('Content-type: application/json'));
    }

    function moveEmail3(string $mailbox, string $id): void
    {
        $this->sendPostRequest($this->baseURL . 'users/' . $mailbox . '/messages/' . $id . '/move', '{ "destinationId": "sentitems" }', array('Content-type: application/json'));
    }

    function sendEmail(string $mailbox, string $subject, string $body, array $toRecipients, array $ccRecipients, array $attachments): string
    {
        $emailData = array(
            'message' => array(
                'subject' => $subject,
        ...
        return $this->sendPostRequest($this->baseURL . 'users/' . $mailbox . '/sendMail', json_encode($emailData), array('Content-type: application/json'));
    }

    function getMailboxes(): array
    {
        $mailboxes = $this->sendGetRequest($this->baseURL . 'users');
        return json_decode($mailboxes['data'], true);
    }

    function basicAddress(array $recipientArray): array
    {
        $addressList = array();

        foreach ($recipientArray as $recipient) {
            $addressList[] = $recipient->emailAddress->address;
        }
        return $addressList;
    }

    function sendGetRequest(string $url, array $headers = null): string
    {
        $ch = curl_init();
    ...
    function sendPostRequest(string $url, string $data, array $headers = null): string
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_POST, 1);
    ...
    function sendDeleteRequest(string $url, array $headers = null): void
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'DELETE');
