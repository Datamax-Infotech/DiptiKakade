...
if (isset($_REQUEST['change_fav_status']) && $_REQUEST['change_fav_status'] == 1) {
	...
	if (isset($_REQUEST["show_urgent_box"]) && $_REQUEST["show_urgent_box"] == 1) {
		...
		if ($fav_inv_ids != "") {
			...
			inventory.material, inventory.customer_pickup_allowed, inventory.uniform_mixed_load, inventory.lead_time, inventory.box_sub_type, inventory.bwall_max,
			inventory.bwall_min,inventory.box_type,inventory.date, inventory.system_description, inventory.quantity_per_pallet, inventory.quantity, inventory.additional_description_text, inventory.location_zip_latitude, inventory.buy_now_load_can_ship_in,
			inventory.location_zip_longitude from inventory WHERE inventory.Active LIKE 'A' and inventory.id in ($fav_inv_ids) order by inventory.b2b_status $availalesort asc";
		}
		...
		inventory.material, inventory.customer_pickup_allowed, inventory.uniform_mixed_load, inventory.lead_time, inventory.box_sub_type, inventory.bwall_max,
		inventory.bwall_min,inventory.box_type,inventory.date, inventory.system_description, inventory.quantity_per_pallet, inventory.quantity, inventory.additional_description_text, inventory.location_zip_latitude, inventory.buy_now_load_can_ship_in,
		inventory.location_zip_longitude from inventory WHERE inventory.Active LIKE 'A' and inventory.id in (" . $_REQUEST["prev_order_str"] . ") order by inventory.b2b_status $availalesort asc";
	} else {
	...
	while ($dt_view_row = array_shift($dt_view_res)) {
		...
		while ($sku_val = array_shift($dt_view_sku)) {
			...
			if ($uniform_mixed_load != "Mixed") {
				...
				$bcubicfootage = round((($blength + isset($blength_frac_conv)) * ($bwidth + isset($bwidth_frac_conv)) * ($bheight + isset($bdepth_frac_conv))) / 1728, 2);
			} else {
			...
			$percent_per_load = 0;
			if ($ftl_qty > 0) {
				$quantity_availableValue_new = (float) str_replace(",", "", $quantity_availableValue_new);
				$ftl_qty = (float) str_replace(",", "", $ftl_qty);

				$percent_per_load = number_format($quantity_availableValue_new / $ftl_qty, 2);
			}
		...
		if (isset($to_show_rec) == "y" && isset($to_show_rec_main1) == "y" && isset($to_show_rec_main2) == "y" && isset($to_show_rec_main3) == "y" && isset($to_show_rec_main4) == "y") {
			...
			if (isset($_REQUEST["shown_in_client_flg"]) && $_REQUEST["shown_in_client_flg"] == 1) {
				...
				//die;
				db();
				$select_fav_status = db_query("SELECT fav_status from b2b_inventory_gaylords_favorite where company_id = '$companyID' and b2b_id = '$b2b_id' and user_id = '" . $user_id . "'");
			...
			if ($_REQUEST["min_price"] == 0 && $_REQUEST["max_price"] == 500 && $_REQUEST["timing"] == "") {
				$products_array[] = array(
					...
					'qtynumbervalue' => isset($quantity_availableValue_new),
					'qtynumbervalueorg' => isset($quantity_availableValue_new),
					'img' => $img,
					'inv_Active' => $dt_view_row["active"],
					'inv_b2b_status' => $dt_view_row["invb2b_status"],
					'inv_box_type' => $dt_view_row["box_type"],
					'boxheight' => $boxheight,
					'boxwidth' => $bwidth,
					'boxlength' => $blength,
					'boxwall' => $boxwall,
					'box_lead_time' => isset($box_lead_time),
					'td_leadtime_bg_color' => isset($td_leadtime_bg_color),
					'box_desc_bottom' => $box_desc_bottom,
					'lead_time_days' => isset($lead_time_days),
					'description' => $description,
					'status' => $status,
					'ship_from' => $ship_from,
					'system_description' => $system_description,
					'flyer_notes' => $flyer_notes,
					'lead_time_of_FTL' => isset($lead_time_for_FTL),
					'distance' => $miles,
					...
					'percent_per_load' => isset($percent_per_load),
