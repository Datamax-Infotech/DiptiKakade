...
Public Class VZ_waterInvoiceControl
    ...
    Private Function FetchData(fromDate As Date, toDate As Date) As DataTable
        ...
        Try
            ...
            SELECT  
                ...
                wic.invoice_no AS InvoiceNumber,
                wt.invoice_date AS InvoiceDate,
                wt.invoice_due_date AS InvoiceDueDate,
        ...
        For Each kvp In widths
            With dataview.Columns(kvp.Key)
                .HeaderText = kvp.Key.Replace("TransactionID", "Trans ID").
                                 ...
                                 Replace("InvoiceDueDate", "Due Date").
        ...
        Try
            ...
            For Each row As DataGridViewRow In dataview.Rows
                ...
                Try
                    ...
                    Dim QBcompanyName As String = row.Cells("VendorName").Value?.ToString()?.Trim()
                    ...
                    If String.IsNullOrWhiteSpace(invoiceNumber) Then
                        isValid = False
                        reasons.Add("❌ Invoice number is blank.")
                    End If
    ...
    Private Sub GridActionClicked(sender As Object, e As DataGridViewCellEventArgs)
        ...
        Select Case clickedColumn
            Case "EditRow"
                ...
                Dim qbComp = r.Cells("VendorName").Value?.ToString()
    ...
    Private Function GetRep(transRecID As Int32) As String
        ...
        Try
            Using conn As New MySqlConnection(connString)
                ...
                Dim query As String = "SELECT rep FROM water_invoice_details WHERE trans_rec_id = @transRecID LIMIT 1"
    ...
    Private Function Getshipmethodvia(transRecID As Integer) As String
        ...
        Try
            Using conn As New MySqlConnection(connString)
                ...
                Dim query As String = "SELECT via FROM water_invoice_details WHERE trans_rec_id = @TransRecID"
                Using cmd As New MySqlCommand(query, conn)
                    cmd.Parameters.AddWithValue("@TransRecID", transRecID)
                    Using reader As MySqlDataReader = cmd.ExecuteReader()
                        If reader.Read() Then
        ...
        Try
            Using conn As New MySqlConnection(connString)
                ...
                Dim query As String = "SELECT quantity, description, price, loop_box_id, division_id, total " &
                                  "FROM water_invoice_items WHERE trans_rec_id = @TransRecID"
    ...
    Private Function GetTermsFromDatabase(buyerID As Integer) As String
        ...
        Try
            ...
            Using conn As New MySqlConnection(connString)
                ...
                Dim query As String = "SELECT terms FROM water_invoice_details WHERE trans_rec_id = @BuyerID"
                Using cmd As New MySqlCommand(query, conn)
    ...
    '            Dim query As String = "SELECT PO FROM water_invoice_details WHERE trans_rec_id = @BuyerID"
    '            Using cmd As New MySqlCommand(query, conn)
    '                cmd.Parameters.AddWithValue("@BuyerID", buyerID)
    ...
    Private Sub SubmitSelectedRows(sender As Object, e As EventArgs)
        ...
        Try
            ...
            Dim templateName As String = InputBox("Enter the invoice template name (e.g., UCBZeroWaste):", "Template Selection", "UCBZeroWaste")
            ...
            For Each row As DataGridViewRow In dataview.Rows
                If row.Cells("Select").Value IsNot Nothing AndAlso Convert.ToBoolean(row.Cells("Select").Value) Then
                    Try
                        ...
                        Dim QBcompanyName As String = If(row.Cells("VendorName").Value?.ToString(), "")
                        ...
                        If row.Cells("InvoiceDueDate").Value IsNot Nothing AndAlso IsDate(row.Cells("InvoiceDueDate").Value) Then
                            dueDate = Convert.ToDateTime(row.Cells("InvoiceDueDate").Value)
                        End If
                        ...
                        Dim success As Boolean = CreateInvoice(sessionManager, msgSetRequest,
                                       buyerID, b2bid, invoiceDate, dueDate, itemDetails,
                                       companyName, QBcompanyName, memo, terms,
    ...
    Private Function CreateInvoice(sessionManager As QBSessionManager,
        ...
        Try
            ...
            'If dueDate <> Date.MinValue Then invoiceAdd.DueDate.SetValue(dueDate)
            ' ✅ Special case: "Due on receipt"
            If Not String.IsNullOrWhiteSpace(terms) AndAlso terms.Trim().Equals("Due on receipt", StringComparison.OrdinalIgnoreCase) Then
                invoiceAdd.DueDate.SetValue(invoiceDate)
            ElseIf dueDate <> Date.MinValue Then
            ...
            ' 🚫 Prevent QuickBooks from marking invoice as "Print Later"
