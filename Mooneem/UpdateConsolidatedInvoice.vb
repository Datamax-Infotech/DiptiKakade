...
Public Class UpdateConsolidatedInvoice
 ...
 STR_TO_DATE(ltb.inv_date_of, '%m/%d/%Y') AS InvoiceDate,
                ...
                WHERE ltb.Leaderboard = 'UCBZW'
                  AND ltb.ignore < 1
                 AND YEAR(STR_TO_DATE(ltb.inv_date_of, '%m/%d/%Y')) > 2024   AND ltb.inv_number LIKE '%R'
            ...
            'hdr.Checked = True
            ' ✅ Prevent default selection of first row
            dataview.ClearSelection()
            dataview.CurrentCell = Nothing
        Catch ex As Exception
    ...
    Private Function UpdateInvoice(sessionManager As QBSessionManager,
        ...
        Try
            ...
            'End If
            ' --- Normalize invoice number (remove revision suffix)
            Dim baseInvoiceNumber As String = invoiceNumber

            If baseInvoiceNumber.EndsWith("-R") Then
                ' Case: invoice ends with "-R"
                baseInvoiceNumber = baseInvoiceNumber.Substring(0, baseInvoiceNumber.Length - 2)
            ElseIf baseInvoiceNumber.EndsWith("R") Then
                ' Case: invoice ends with just "R"
                baseInvoiceNumber = baseInvoiceNumber.Substring(0, baseInvoiceNumber.Length - 1)
            End If
