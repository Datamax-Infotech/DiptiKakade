    ...
    Private Sub LoadData()
        Try
            ...
            SELECT  
                ...
                ww.Name AS QuickBooksCompanyName,
            ...
            WHERE ww.active_flg = 1
                ...
                AND wt.amount > 0
                AND DATE(wt.transaction_date) > 2024'
            ORDER BY wt.id DESC;"
    ...
    Private Function GetInvoiceDate(transRecID As Integer) As String
        ...
        Try
            Using conn As New MySqlConnection(connString)
                ...
                Dim query As String = "SELECT timestamp FROM water_invoice_details WHERE trans_rec_id = @TransRecID"
                Using cmd As New MySqlCommand(query, conn)
                    cmd.Parameters.AddWithValue("@TransRecID", transRecID)
                    Using reader As MySqlDataReader = cmd.ExecuteReader()
    ...
    Private Function UpdateInvoice(sessionManager As QBSessionManager,
        ...
        Try
            ...
            If Not String.IsNullOrWhiteSpace(terms) AndAlso terms.Trim().Equals("Due on receipt", StringComparison.OrdinalIgnoreCase) Then
                invoiceMod.DueDate.SetValue(invoiceDate)
            ElseIf dueDate <> Date.MinValue Then
                invoiceMod.DueDate.SetValue(dueDate)
            End If
