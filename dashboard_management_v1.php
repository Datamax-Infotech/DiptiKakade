	...
	<div id="content-wrapper" class="d-flex flex-column">
		...
		<div class="container-fluid">
			...
			<div class="row">   
                                        ...
                                        <tr>
                                            ...
                                            if(isset($scorecardweeks_for_thead)){
                                                foreach($scorecardweeks_for_thead as $week){
                                                    ?>
                                                <th class="f-small"><?php echo $week?></th>
                                                <?php
                                                }
                                            }
                                            ?>
                                        ...
                                        <tr>
                                            <td class="td-border-bottom text-left">
                                                <a id="measurableModal" type="button" class="" data-toggle="modal"
                                                    ...
                                                    data-whatever="<?php echo new_dash_encrypt($scorecard_data['id'])?>" data-todo='{"EditingFrom":"MeasurablePage"}'><?php echo $scorecard_data['name']?></a>
                                            </td>
                                            <td class="td-border-bottom ">
											
                                                <?php echo $scorecard_data['goal'] == '==' ? '=' : $scorecard_data['goal']?>
                                                <?php echo $scorecard_data['units'] === '%' ? $scorecard_data['goal_matric'].$scorecard_data['units'] : $scorecard_data['units'] . $scorecard_data['goal_matric']; ?>
                                            </td>
                                            <?php
                                            if(isset($scorecardweeks_for_thead)){
                                                ...
                                                foreach($scorecardweeks_for_thead as $week){
                                                    ...
                                                    if(!empty($inner_scorecard_data_query)){
                                                        while($inner_scorecard_data = array_shift($inner_scorecard_data_query)){
															...
															$onblur = ""; $putValue = false; $inputValue = ''; $box_color = '';
                                                            ...
                                                            ?>
                                                                <td class="measurable_val_td <?php echo $putValue == true ? $box_color : '' ?>">
                                                                    <input type="text" class="edit_content text-center" onblur="<?php echo $onblur?>" onkeyup="matrixvalidateInput(this)" value="<?php echo $putValue == true ? $inputValue : '' ?>" />
                                                                    <input type="hidden" class="tdweek" value="<?php echo str_replace('<br>', " to " , $week); ?>">
                                                                </td>
                                                            <?php
                                                       }
                                                    }else{
                                                        ...
                                                        ?>
                                                            <td class="measurable_val_td">
                                                                <input type="text" class="edit_content text-center" onblur="<?php echo $onblur?>" onkeyup="matrixvalidateInput(this)" value="" />
                                                                <input type="hidden" class="tdweek" value="<?php echo str_replace('<br>', " to " , $week); ?>">
                                                            </td>
                                                        <?php
                                                    }
                                            ...
                                            ?>
                                            <input type="hidden" class="trID" value="<?php echo new_dash_encrypt($scorecard_data['id']) ?>">
                                        </tr>
                                        <?php
                                        }
                                        ?>
					...
					<div class="card shadow mb-4">
						...
						<div class="card-body fixed-height-card" id="project_table">
							<?php
								...
								//$sql_main = db_query("SELECT id, meeting_name FROM meeting_master where status = 1 union SELECT 0, 'No Meeting selected' ORDER BY id DESC");
								...
								while($main_row = array_shift($sql_main)){
									...
									if($count>0){
										...
										?>
										<table class="vertical-middle table table-sm border-0 <?php echo "project_meeting".$main_row['id'];?>" width="100%" cellspacing="0">
											<tbody>
												<tr>
													<td colspan="3">
														<div class="card-subheading"><span><?php echo $main_row["meeting_name"];?></span></div>
													</td>
												</tr>
												<?php 
													$sql1 = db_query("SELECT project_id,project_name, project_description,project_status_id, project_deadline FROM project_master where find_in_set($meeting_id,meeting_ids) AND project_owner = '".$_COOKIE["b2b_id"]."' AND archive_status=0 ORDER BY project_id DESC LIMIT 10");
					...
					<div class="card shadow mb-4">
						...
						<div class="card-body fixed-height-card"  id="task_table">
							...
							<?php
								...
								//$sql_main = db_query("SELECT id, meeting_name FROM meeting_master where status = 1 ORDER BY id DESC");
								...
								while($main_row = array_shift($sql_main)){
									...
									//$count_sql = db_query("SELECT id FROM task_master where task_meeting=$meeting_id AND task_assignto = '" . $_COOKIE["b2b_id"]. "' and archive_status=0 and task_master.task_status = 0 ORDER BY id DESC");
									$count_sql = db_query("SELECT id FROM task_master where task_meeting=$meeting_id AND task_assignto = '" . $_COOKIE["b2b_id"]. "' and archive_status=0 ORDER BY id DESC");
									$count = tep_db_num_rows($count_sql);
									if($count>0){ $check_for_no_data=false;?>
										<table class="table table-sm border-0 <?php echo "task_meeting".$main_row['id'];?>" width="100%" cellspacing="0">
											<tbody>
												<tr>
													<td colspan="3">
														<div class="card-subheading"><span><?php echo $main_row["meeting_name"];?></span></div>
												...
												<?php 
													...
													and archive_status=0 ORDER BY id DESC LIMIT 10");
													while($r = array_shift($sql1)){
														?>
				...
				//$sql_main = db_query("SELECT id, meeting_name FROM meeting_master where status = 1 and id!=0 ORDER BY id ASC");
               $sql_main = db_query("SELECT mm.id, mm.meeting_name FROM meeting_attendees as ma JOIN meeting_master as mm ON mm.id=ma.meeting_id 
				where mm.status = 1 $meeting_filter and mm.id != 0 GROUP By ma.meeting_id ORDER BY meeting_name ASC, attendee_id< '".$_COOKIE['b2b_id']."' ASC,attendee_id");
				$check_first=1;
				 while($main_row = array_shift($sql_main)){
					$meeting_id=$main_row['id'];
					$count_sql=db_query("SELECT id FROM issue_master where meeting_id=$meeting_id and status=1 ORDER BY id DESC");
					$count=tep_db_num_rows($count_sql);				
				?>
				<div class="col-lg-6 issueList" <?php echo $check_first==1 ? "id='issue-section'" : "";?>>
					<div class="card shadow mb-4">
						<!-- Card Header - Dropdown -->
						<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
							...
							<div class="dropdown no-arrow">
								<span class="show-mine dropdown-toggle mx-1">
									<input type="checkbox" class="form-control-check only-mine-check" id="show_only_<?php echo $meeting_id; ?>" meeting_id="<?php echo $meeting_id;?>" name="only_mine" value="yes"/> Show Only Mine
								</span>
								...
								<div class="dropdown-menu mx-2 dropdown-menu-right shadow animated--fade-in"
				 				...
				 				-->
								 <a id="new_issue_tile" href="javascript:add_issue(<?php echo $meeting_id;?>)" class="mx-1"><i class="fa fa-plus"></i></a>
							</div>
						</div>
						<!-- Card Body -->
						<div class="card-body fixed-height-card" id="meeting_issue_<?php echo $meeting_id; ?>">
							<?php if($count>0){?>
							<div class="table-responsive">
								<table class="table table-sm border-0" >
									<tbody>
										<?php 
											...
											$sql1 = db_query("SELECT issue_master.id,issue,issue_master.created_by from issue_master where meeting_id=$meeting_id and issue_master.status=1 ORDER BY id DESC LIMIT 10");
											while($r = array_shift($sql1)){
												db();
												$select_emp=db_query("SELECT Headshot, name,initials from loop_employees where b2b_id='".$r['created_by']."'");
												$emp_data = array_shift($select_emp);
												$empDetails=getOwerHeadshotForMeeting($emp_data['Headshot'],$emp_data['initials']); 
												db_project_mgmt();
										?>
										<tr id="issue_tr_<?php echo $r['id'];?>">
											<td><span class="attendees_img" style="background-image:url('<?php echo $empDetails['emp_img']; ?>')"><?php echo $empDetails['emp_txt']; ?></span></td>
											<!--<td><img class="img-profile rounded-circle" src="<?php //echo $emp_img; ?>" height="30" width="30"></td>-->
