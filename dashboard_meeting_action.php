...
db_project_mgmt();
function getAttendeeInfo(int|String $meeting_id): array{

	$sql_main = db_query("SELECT ma.id, ma.attendee_id FROM meeting_attendees as ma where ma.meeting_id= $meeting_id ORDER BY id ASC");
	$result = array();
	while ($hrow = array_shift($sql_main)) {
		db();
		$empDetails_qry = db_query("SELECT Headshot, b2b_id,name,initials from loop_employees where b2b_id='" . $hrow['attendee_id'] . "'");
...
function getPageInfo(int|string $meeting_id): array
{

	$sql_main = db_query("SELECT page_id,page_type,page_title,duration,order_no FROM meeting_pages where meeting_id= $meeting_id ORDER BY order_no ASC");
	$result = array();
	while ($hrow = array_shift($sql_main)) {
		$result[] = $hrow;
	}
...
function getStartedMeetingsForNotification(int|string $emp_level = 0): array
{
	$meeting_filter = "";
	if ($emp_level != 2) {
...
function update_page_action_on_live_update(string $current_page, int $meeting_timer_id): void {

	$updated_data_of = "";
	switch ($current_page) {
		case "meeting_projects.php":
			$updated_data_of = ",project_flg=0";
			break;
		case "meeting_metrics.php":
			$updated_data_of = ",metrics_flg=0";
			break;
		case "meeting_tasks.php":
			$updated_data_of = ",task_flg=0";
			break;
		case "meeting_issues.php":
			$updated_data_of = ",issue_flg=0";
			break;
		case "meeting_conclude.php":
			$updated_data_of = ",rating_flg=0";
	}
	$attendee_id = $_COOKIE['b2b_id'];
	db_query("UPDATE meeting_live_updates set current_page='" . $current_page . "' $updated_data_of where meeting_timer_id= $meeting_timer_id && attendee_id= $attendee_id");
}
if (isset($_GET['edit_page']) && $_GET['edit_page'] == 1) {
	$page_id = $_GET['page_id'];

	$sql1 = db_query("SELECT page_id,page_type,page_title,duration,	page_subheading FROM meeting_pages where page_id = $page_id ");
	$data = [];
	while ($r = array_shift($sql1)) {
		$data = $r;
	}
...
if (isset($_POST["meeting_action"]) && $_POST["meeting_action"] == "save-meeting-name") {
	...
	if ($meeting_id == "") {
		$meeting_exist_count_sql = db_query("SELECT id FROM meeting_master where  meeting_name = '" . $meeting_name . "'");
...
if (isset($_POST["meeting_attendees_list"]) && $_POST["meeting_attendees_list"] == 1) {
	...
	$att_sql_main = db_query('SELECT attendee_id from meeting_attendees where meeting_id ="' . $_POST['meeting_id'] . '"');
...
if (isset($_POST["delete_meeting_attendees"]) && $_POST["delete_meeting_attendees"] == 1) {
	...
	$result = db_query($sql_delete_old_milestone);
	$res = getAttendeeInfo($_POST['meeting_id']);
	echo json_encode($res);
}
...
if (isset($_POST["page_action"]) && $_POST["page_action"] != "") {
	...
	if ($_POST["page_action"] == "DELETE") {
		...
		$last_inserted_order = db_query("SELECT order_no FROM `meeting_pages` WHERE meeting_id= $meeting_id order by order_no DESC limit 1");
...
if (isset($_POST['fav_meeting_action']) and $_POST['fav_meeting_action'] == 1) {
	...
	$check_already_marked = db_query("SELECT id,fav_status FROM meeting_favourite_mark where meeting_id= $meeting_id && attendee_id='" . $_COOKIE['b2b_id'] . "'");
	if (tep_db_num_rows($check_already_marked) > 0) {
		while ($row = array_shift($check_already_marked)) {
			db_query("UPDATE `meeting_favourite_mark` set fav_status= $fav_status where id='" . $row['id'] . "'");
		}
	} else {
		db_query("INSERT INTO `meeting_favourite_mark` ( `meeting_id`, `attendee_id` ,`fav_status`) VALUES ($meeting_id,'" . $_COOKIE['b2b_id'] . "',$fav_status)");
	}
	echo 1;
}
if (isset($_REQUEST['page_sort_action']) and $_REQUEST['page_sort_action'] == "updateSortedRowsOfPage") {
	...
	foreach ($newOrder as $id) {
		...
		db_query('UPDATE meeting_pages SET order_no="' . $n . '" WHERE page_id="' . $id . '" ');
		$n++;
	}
	echo 1;
}
if (isset($_GET['page_change_action']) and $_GET['page_change_action'] == 'update_page_change') {
	...
	if ($_COOKIE['b2b_id'] == get_meeting_owner($meeting_timer_id)) {
		$res = 1;
		db_query("UPDATE meeting_live_updates set owner_page_flg=1 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id!=" . $_COOKIE['b2b_id']);
	}
	echo $res;
}

if (isset($_GET['update_owner_page_time_counter']) and $_GET['update_owner_page_time_counter'] == 1) {
	db_query("UPDATE meeting_time_spent set time_spent='" . $_GET['time_spent'] . "' where id=" . $_GET['table_page_id']);
	db_query("UPDATE meeting_live_updates set owner_page_timer_flg=1 where meeting_timer_id='" . $_GET['meeting_timer_id'] . "' && attendee_id!=" . $_COOKIE['b2b_id']);
	echo 1;
}
...
if (isset($_REQUEST['finish_meeting_action']) and $_REQUEST['finish_meeting_action'] == 1) {
	...
	$result = db_query($update_task_sql);
	...
	db_query($update_task_flag_during_meeting_sql);
	...
	if ($send_meeting_email_to == 1 || $send_meeting_email_to == 2) {
		...
		$meeting_name_qry = db_query("SELECT meeting_name FROM meeting_master where id = $meeting_id");
		...
		$task_sql = db_query("SELECT task_duedate,task_title, task_assignto FROM task_master where task_meeting= $meeting_id and archive_status=0 ORDER BY task_master.id DESC");
		if (tep_db_num_rows($task_sql) > 0) {
			$email_msg .= '<table class="table_css"><tbody>';
			while ($r = array_shift($task_sql)) {
				db();
				$empDetails_qry = db_query("SELECT name from loop_employees where b2b_id='" . $r['task_assignto'] . "'");
		...
		if (tep_db_num_rows($qry_issue) > 0) {
			...
			while ($r = array_shift($qry_issue)) {
				if ($r['status'] == 0) {
					db();
					$empDetails_qry = db_query("SELECT name from loop_employees where b2b_id='" . $r['created_by'] . "'");
		...
		$attendee_qry = db_query("SELECT attendee_id FROM meeting_start_atten_ratings as ms where ms.meeting_timer_id= $meeting_timer_id $rating_filter");
		$rec_array = [];
		while ($r = array_shift($attendee_qry)) {
			db();
			$empDetails_qry = db_query("SELECT email, name from loop_employees where b2b_id='" . $r['attendee_id'] . "'");
			$rec_array[] = array_shift($empDetails_qry)['email'];
		}
	...
	db_query("UPDATE meeting_live_updates set meeting_flg=1 where meeting_timer_id=" . $meeting_timer_id);
	db_query("UPDATE meeting_live_updates set meeting_flg=2 where meeting_timer_id=" . $meeting_timer_id . " && attendee_id=" . $_COOKIE['b2b_id']);
...
if (isset($_GET['leave_meeting_action']) and $_GET['leave_meeting_action'] == 1) {
	...
	$result = db_query($upsql);
	db_query("UPDATE meeting_live_updates set attendee_flg=1 where meeting_timer_id=" . $_GET['meeting_timer_id']);
...
if (isset($_GET['update_rating']) and $_GET['update_rating'] == 1) {
	$meeting_timer_id = $_GET['meeting_timer_id'];
	db_query('UPDATE meeting_start_atten_ratings set rating = "' . $_GET['rating'] . '" where id="' . $_GET['rating_table_id'] . '"');
	update_meeting_minutes($_GET['meeting_id'], $meeting_timer_id, 'Rating', 'Rating Done', $_GET["rating_table_id"], $_COOKIE['b2b_id']);
	db_query("UPDATE meeting_live_updates set rating_flg=1 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id!='" . $_COOKIE['b2b_id'] . "'");
...
if (isset($_GET['hide_notification_for_meeting_start_updates']) && $_GET['hide_notification_for_meeting_start_updates'] != "") {
	if ($_GET['hide_notification_for_meeting_start_updates'] == 1) {
		$sql1 = db_query("UPDATE meeting_start_atten_ratings set notification_status=1 where attendee_id=" . $_COOKIE['b2b_id'] . " && meeting_timer_id=" . $_GET['meeting_timer_id']);
	} else {
		$sql1 = db_query("UPDATE meeting_start_atten_ratings set notification_status=2 where attendee_id=" . $_COOKIE['b2b_id'] . " && meeting_timer_id=" . $_GET['meeting_timer_id']);
	}
...
if (isset($_GET['get_live_meeting_status_updates']) && $_GET['get_live_meeting_status_updates'] == 1) {
	...
	if (isset($_GET['data_of']) && $_GET['data_of'] == 'all') {
		$meeting_filter = "";
		if ($_GET['emp_level'] != 2) {
			$meeting_filter = " and ma.attendee_id='" . $_COOKIE['b2b_id'] . "'";
		}
		$updated_meetings_qry = db_query("SELECT mt.meeting_id,mt.id,mt.meeting_flg from meeting_timer as mt JOIN meeting_attendees as ma ON mt.meeting_id= ma.meeting_id where $com_str $meeting_filter GROUP By ma.meeting_id");
	...
	if ($_GET['data_of'] == 'single') {
		...
		$updated_meetings_qry = db_query("SELECT mt.meeting_id,mt.id,mt.meeting_flg from meeting_timer as mt where $com_str and meeting_id= $meeting_id ORDER BY id DESC limit 1");
		if (tep_db_num_rows($updated_meetings_qry) > 0) {
			while ($row = array_shift($updated_meetings_qry)) {
				$result['meeting_id'] = $row['meeting_id'];
				$result['meeting_timer_id'] = $row['id'];
...
if (isset($_GET['get_live_data_of_meeting']) && $_GET['get_live_data_of_meeting'] == 1) {
	...
	$flg_update = db_query("SELECT * FROM meeting_live_updates where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id='" . $attendee_id . "'");
	...
	if ($current_page == "meeting_projects.php" && $update_data_array['project_flg'] == 1) {
		...
		db_query("UPDATE meeting_live_updates set project_flg=0 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id='" . $attendee_id . "'");
	}
	...
	if ($current_page == "meeting_tasks.php" && $update_data_array['task_flg'] == 1) {
		...
		db_query("UPDATE meeting_live_updates set task_flg = 0 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id='" . $attendee_id . "'");
		switch ($order_str) {
			case "ORDER BY task_assignto ASC":
				$sort_task = 1;
				break;
			case "ORDER BY task_status ASC":
				$sort_task = 2;
				break;
			case "ORDER BY task_duedate DESC":
				$sort_task = 3;
				break;
			case "ORDER BY task_duedate ASC":
				$sort_task = 4;
				break;
			case "ORDER BY task_entered_on ASC":
				$sort_task = 5;
				break;
			case "ORDER BY task_entered_on DESC":
				$sort_task = 6;
		}
	}
	...
	if ($current_page == "meeting_issues.php" && $update_data_array['issue_flg'] == 1) {
		...
		db_project_mgmt();
		db_query("UPDATE meeting_live_updates set issue_flg=0 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id='" . $attendee_id . "'");
		switch ($order_str) {
			case "ORDER BY order_no<1 ASC,order_no":
				$sort_issue = 1;
				break;
			case "ORDER BY created_by ASC":
				$sort_issue = 2;
				break;
			case "ORDER BY created_on ASC":
				$sort_issue = 3;
				break;
			case "ORDER BY created_on DESC":
				$sort_issue = 4;
				break;
			case "ORDER BY issue ASC":
				$sort_issue = 5;
		}
	}
	$metrics_flg = 0;
	...
	if ($current_page == "meeting_metrics.php" && $update_data_array['metrics_flg'] == 1) {
		...
		db_project_mgmt();
		db_query("UPDATE meeting_live_updates set metrics_flg=0 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id='" . $attendee_id . "'");
	}
	...
	if ($current_page == "meeting_conclude.php" && $update_data_array['rating_flg'] == 1) {
		...
		db_project_mgmt();
		db_query("UPDATE meeting_live_updates set rating_flg=0 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id='" . $attendee_id . "'");
	}
	$owner_current_page = "";
	$owner_page_flg = 0;
	if ($update_data_array['owner_page_flg'] == 1) {
		$owner_page_flg = 1;
		db_project_mgmt();
		$current_page_of_owner_sql = db_query("SELECT current_page from meeting_live_updates where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id=" . get_meeting_owner($meeting_timer_id));
		$owner_current_page = array_shift($current_page_of_owner_sql)['current_page'];
		db_query("UPDATE meeting_live_updates set owner_page_flg=0 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id='" . $attendee_id . "'");
	}
	...
	if ($update_data_array['owner_page_timer_flg'] == 1) {
		...
		db_project_mgmt();
		db_query("UPDATE meeting_live_updates set owner_page_timer_flg=0 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id='" . $attendee_id . "'");
		$select_time = db_query("SELECT id,time_spent from meeting_time_spent where meeting_timer_id=" . $meeting_timer_id);
	...
	if ($update_data_array['attendee_flg'] == 1) {
		...
		db_project_mgmt();
		db_query("UPDATE meeting_live_updates set attendee_flg=0 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id='" . $attendee_id . "'");
	}
	...
	if ($update_data_array['meeting_flg'] == 1) {
		$meeting_flg = 2;
		db_query("UPDATE meeting_live_updates set meeting_flg=2 where meeting_timer_id='" . $meeting_timer_id . "' && attendee_id='" . $attendee_id . "'");
...
if (isset($_REQUEST['matircs_sort_action']) && $_REQUEST['matircs_sort_action'] == "matricsTableOrdering") {
	if (isset($_REQUEST['meetingTimerID'])) {
		db_query("UPDATE meeting_live_updates set metrics_flg=1 where meeting_timer_id=" . new_dash_decrypt($_REQUEST['meetingTimerID']) . " AND attendee_id!=" . $_COOKIE['b2b_id'] . "");
	}
	$newOrder = explode(",", $_REQUEST['sortOrder']);
	$n = 1;
	foreach ($newOrder as $id) {
		db_query('UPDATE scorecard SET meeting_create_order_no=' . $n . ' WHERE id=' . new_dash_decrypt($id) . ' ');
		$n++;
	}
	echo 1;
}
...
if (isset($_REQUEST['das_meeting_matrics']) and $_REQUEST['das_meeting_matrics'] == "deleteMatrix") {
	...
	if (isset($encryptedDeleteID) && $encryptedDeleteID != '' && isset($meetingID) && $meetingID != 0) {
		...
		$fetchDelete_query = db_query($fetchDeleteSQL);
			...
			if ((int)$fetchDeleteData['id'] === (int)$deleteID) {
				...
				if (isset($explodedDeletedValue) && isset($explodedDeletedValue[1])) {
					...
					if (isset($outputString) &&  $outputString != '') {
						...
						$sqlUpdate_query = db_query($sqlUpdate);
						// echo "Updating";
					}
				} else {
					if (isset($explodedDeletedValue[0]) &&  $explodedDeletedValue[0] != '') {
						...
						$singleMeetingSql_query = db_query($singleMeetingSql);
						// echo "Deleting";
					}
				}
...
if (isset($_GET['archive_meeting']) && $_GET['archive_meeting'] == 1) {
	db_query("UPDATE meeting_master set status=0 where id=" . $_GET['meeting_id']);
	echo 1;
}

if (isset($_GET['edit_vto']) && $_GET['edit_vto'] == 'usedcarboardboxes_vto') {
	$vto_data_sql = db_query("SELECT id,title,description FROM usedcarboardboxes_vto where id=" . $_GET['id']);
	$data = [];
...
if (isset($_POST['update_vto']) && $_POST['update_vto'] == 'usedcarboardboxes_vto') {
	...
	db_query($insql);
	$vto_data_sql = db_query("SELECT id,title,description FROM usedcarboardboxes_vto where id=" . $_POST['id']);
	$data = [];
	while ($r = array_shift($vto_data_sql)) {
		$data = $r;
	}
...
if (isset($_GET['edit_vto']) && $_GET['edit_vto'] == 'ucbzerowaste_vto') {
	$vto_data_sql = db_query("SELECT id,title,description FROM ucbzerowaste_vto where id=" . $_GET['id']);
	$data = [];
	while ($r = array_shift($vto_data_sql)) {
		$data = $r;
	}
...
if (isset($_POST['update_vto']) && $_POST['update_vto'] == 'ucbzerowaste_vto') {
	...
	db_query($insql);
	$vto_data_sql = db_query("SELECT id,title,description FROM ucbzerowaste_vto where id=" . $_POST['id']);
	$data = [];
	while ($r = array_shift($vto_data_sql)) {
		$data = $r;
	}
...
if (isset($_GET['edit_vto']) && $_GET['edit_vto'] == '2ndkid_vto') {
	$vto_data_sql = db_query("SELECT id,title,description FROM 2ndkid_vto where id=" . $_GET['id']);
	$data = [];
	while ($r = array_shift($vto_data_sql)) {
		$data = $r;
	}
...
if (isset($_POST['update_vto']) && $_POST['update_vto'] == '2ndkid_vto') {
	...
	db_query($insql);
	$vto_data_sql = db_query("SELECT id,title,description FROM 2ndkid_vto where id=" . $_POST['id']);
	$data = [];
	while ($r = array_shift($vto_data_sql)) {
		$data = $r;
	}
...
if (isset($_GET['edit_meeting_vto']) && $_GET['edit_meeting_vto'] == 1) {
	...
	$vto_data_sql = db_query("SELECT id $select_str FROM meeting_vto where id=" . $_GET['id']);
	$data = [];
	while ($r = array_shift($vto_data_sql)) {
		$data = $r;
	}
...
if (isset($_POST['update_meeting_vto']) && $_POST['update_meeting_vto'] == 1) {
	...
	db_query("Update `meeting_vto` set $update_str where id = " . $_POST['id']);
	$vto_data_sql = db_query("SELECT * FROM meeting_vto where id=" . $_POST['id']);
	$data = [];
	while ($r = array_shift($vto_data_sql)) {
		$data = $r;
	}
...
if (isset($_GET['edit_core_value']) && $_GET['edit_core_value'] == 1) {
	$core_values_data_sql = db_query("SELECT id,title,description FROM core_values where id=" . $_GET['id']);
	$data = [];
...
if (isset($_POST['update_core_value']) && $_POST['update_core_value'] == 1) {
	...
	db_query($insql);
	$vto_data_sql = db_query("SELECT id,title,description FROM core_values where id=" . $_POST['id']);
	$data = [];
	while ($r = array_shift($vto_data_sql)) {
		$data = $r;
	}
