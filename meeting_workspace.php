	...
	<div id="content-wrapper" class="d-flex flex-column">
		...
		<div class="container-fluid">
			...
			<div class="d-sm-flex align-items-center justify-content-between my-4">
				<h1 class="h3 mb-0">Workspace : <?php echo  $meeting_name; ?></h1>
			...
			<div class="row">   	
                <div class="col-md-12" id="scoreboard-section">
                    <div class="card shadow mb-4">
						<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
							<h6 class="collapse-heading">Scorecard <small><i><?php echo  $meeting_name;?></i></small></h6>
                            ...
                            <div class="table-responsive">
                                <table id="meetingMetricsTable" class="table table-sm text-center meetingMetricsTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr data-sort-id="<?php echo new_dash_encrypt($scorecard_weeks_id)?>">
                                                <td class="matrics_attandees_img"><span class="attendees_img" style="background-image:url('<?php echo $scorecardUserImage?>')"><?php echo $scorecardUserText?></span></td>
                                                <td class="td-border-bottom matrics_mesurable_name">
                                                    <a id="measurableModal" type="button" class="" data-toggle="modal"
                                                        ...
                                                        data-whatever="<?php echo new_dash_encrypt($scorecard_weeks_id)?>" data-todo='{"EditingFrom":"meetingWorkspaceMatrix"}'><?php echo $scorecard_data['name']?></a>
                                                </td>
                                                <td class="td-border-bottom matrics_mesurable_goal">
                                                    <?php echo $scorecard_data['goal'] == '==' ? '=' : $scorecard_data['goal']?>
                                                    <?php echo $scorecard_data['units'].$scorecard_data['goal_matric']?>
                                                </td>
                                                    ...
                                                    foreach($scorecardweeks_for_thead as $week){
                                                        ...
                                                        if(!empty($inner_scorecard_data_query)){
                                                            while($inner_scorecard_data = array_shift($inner_scorecard_data_query)){
																...
																$putValue = true; $onblur = ""; $inputValue = ""; $box_color = "";
				...
				<div class="col-lg-6  projects" id="project-section">
					<div class="card shadow mb-4">
						...
						<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
							<h6 class="collapse-heading">Projects <small><i><?php echo  $meeting_name;?></i></small></h6>
							<div class="dropdown no-arrow">
						...
						<div class="card-body fixed-height-card" >
							<table id="project_table" class="vertical-middle table table-sm border-0 <?php echo "project_meeting"; ?>" width="100%" cellspacing="0">
								<tbody>
									<?php 
									db_project_mgmt();
										$sql1 = db_query("SELECT project_id,project_name, project_description,project_status_id, project_deadline,project_owner FROM project_master where find_in_set($meeting_id,meeting_ids) AND archive_status=0 ORDER BY project_id DESC ");
										if(tep_db_num_rows($sql1)>0){
										while($row = array_shift($sql1)){
											
											db();
											$empDetails_qry = db_query("SELECT Headshot, name,initials from loop_employees where b2b_id='".$row['project_owner']."'");
											$empDetails_arr=array_shift($empDetails_qry);
											$empDetails=getOwerHeadshotForMeeting($empDetails_arr['Headshot'],$empDetails_arr['initials']); 
									?>
				...
				<div class="col-lg-6 tasks" id="task-section">
					<div class="card shadow mb-4">
						<!-- Card Header - Dropdown -->
						<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
							<h6 class="collapse-heading">Tasks <small><i><?php echo  $meeting_name;?></i></small></h6>
							<div class="dropdown no-arrow">
								<span class="show-mine dropdown-toggle mx-1">
						...
						<div class="card-body fixed-height-card"  >
							<div class="table-responsive">
							<table id="task_table" class="table table-sm border-0 <?php echo "task_meeting"; ?>" width="100%" cellspacing="0">
								<tbody>
									<?php 
									db_project_mgmt();
										$sql1 = db_query("SELECT task_master.id,task_title, task_duedate, task_status,task_entered_by, task_assignto FROM task_master where task_meeting = $meeting_id  and archive_status=0 ORDER BY id DESC ");
										if(tep_db_num_rows($sql1)>0){
											while($row = array_shift($sql1)){
												db();
												$empDetails_qry = db_query("SELECT Headshot, name,initials from loop_employees where b2b_id='".$row['task_assignto']."'");
												$empDetails_arr=array_shift($empDetails_qry);
												$empDetails=getOwerHeadshotForMeeting($empDetails_arr['Headshot'],$empDetails_arr['initials']);  
												?>
				...
				<div class="col-lg-6 issueList" id="issue-section">
					<div class="card shadow mb-4">
						<!-- Card Header - Dropdown -->
						<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
							<h6 class="collapse-heading"> Issue: <?php echo  $meeting_name;?></h6>
							<div class="dropdown no-arrow">
								<span class="show-mine dropdown-toggle mx-1">
								...
								<table class="table table-sm border-0"  id="meeting_issue_tbl" >
									<tbody>
										...
										db_project_mgmt();
											...
											where meeting_id = $meeting_id and issue_master.status=1 ORDER BY id DESC");
											if(tep_db_num_rows($sql1)>0){
												db();
											while($row = array_shift($sql1)){
												$empDetails_qry = db_query("SELECT Headshot, name,initials from loop_employees where b2b_id='".$row['created_by']."'");
												$empDetails_arr=array_shift($empDetails_qry);
												$empDetails=getOwerHeadshotForMeeting($empDetails_arr['Headshot'],$empDetails_arr['initials']); 
										?>
										<tr id="issue_tr_<?php echo $row['id'];?>">
