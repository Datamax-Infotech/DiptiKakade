...

{
	...
	if(isset($_REQUEST["stag"]) && ($_REQUEST["stag"]!="")){
		...
		if(isset($_REQUEST["stag"]) && ($_REQUEST["stag"]!="")){
			...
			$total_tag = tep_db_num_rows($stag_arr);
...

{
...

	{
		...
		$x=0; $newflg = "no"; $preordercnt = 1; $filter_subtype = ""; $inv_id_list = "";
		...
		foreach ($box_type_str_arr as $box_type_str_arr_tmp){
			...
			if ($box_type_cnt == 1){ $box_type = "Shipping Boxes"; }

			if ($box_type_cnt == 2){ $box_type = "Pallets"; }
...

			{
...

				{
					...
					$tmp_noofpallet = 0; $ware_house_boxdraw = ""; $preorder_txt = ""; $preorder_txt2 = ""; 

					$box_warehouse_id = 0; $next_load_available_date = "";
...
?>
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="1" class="basic_style">
			...
			?>
					...
					<tr><td class="display_maintitle" align="center">Active Inventory Items - <?php echo $boxtype; ?></td></tr>
					...
					<?php
						...
						$count_arry=0; $count=0; $row_cnt=0;
...
<?php }else {?>
	...
	<?php 
		...
		$whileLoopIteration = 0; $g_item_sub_type = 0;

		$gr_total_no_of_loads = 0; $gr_no_of_loads = 0; 

		$gr_all_load_cost = 0; $gr_avl_load_cost = 0;
			...
			while ($qgb = array_shift($qdt_view_res)) {
				...
				if ($_REQUEST["load_all"] == 1){
					...
					elseif($_REQUEST["display-allrec"] == 2){
						...
						if ($_REQUEST["sort_g_tool"] == "2"){
							...
							$b2b_status_str = "(b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2 or b2b_status=2.8)";
						...
						}else if ($_REQUEST["dashpg"] == "high_value_target"){

							$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, 

							inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE high_value_target=1 ORDER BY inventory.availability DESC";
				...
				while ($inv = array_shift($yyyy)) {
					...
					if ( ($g_item_sub_type <> 11 && $g_item_sub_type > 0) || ($_REQUEST["sort_g_tool2"] == 2) ||($show_rec_condition1 == "yes" && $show_rec_condition2 == "yes" && $show_rec_condition3 == "yes" && $show_rec_condition4 == "yes" && $show_rec_condition5 == "yes" && $show_rec_condition6 == "yes")) {
						...
						$ship_cdata_ltl = 'No'; $pickup_cdata_allowed = 'No'; $flyer_notes = "";
						...
						if($ship_ltl == 1){ $ship_cdata_ltl = 'Yes';}

						if($customer_pickup_allowed == 1){ $pickup_cdata_allowed = 'Yes';}
...

						{
							...
							$next_load_available_date = ""; $shipfrom_comp = ""; $previous_contents = "";
							...
							while ($loc_res = array_shift($dt_view)) {
								...
								$get_loc_qry = "Select nickname from companyInfo where loopid = " . $vendor_b2b_rescue_id;
...

							{
								...
								elseif($st_row["status_key"]=="2.0" || $st_row["status_key"]=="2.1" || $st_row["status_key"]=="2.2" || $st_row["status_key"]=="2.8"){
						...
						//
								...
								if (isset($_REQUEST["sort_g_location"]) && ($_REQUEST["sort_g_location"]=="2" || $_REQUEST["sort_g_location"]=="3" || $_REQUEST["sort_g_location"]=="4" )) {
									...
									if (isset($_REQUEST["display_view"])) { 
										...
										$avl_load_total_cost = $total_no_of_loads * $boxes_per_trailer * $b2b_fob_main;

										$avl_load_cost = $no_of_loads * $boxes_per_trailer * $b2b_fob_main;
										...
										$expand_btn=""; $expand_btn_collapse ="";
										...
										<a target='_blank' href='https://ucbloops.com/b2b/?id=" . urlencode(encrypt_password($inv["loops_id"])) . "'>

										<img alt='' src='https://ucbloops.com/loops/boxpics_thumbnail/" . $bpic_1 . "' width='120px' height='120px;' style='object-fit: cover;'/></a></td>";
										...
										if ($_REQUEST["autoloadflg"] == 1){

											$makctrl_unique = "ctrauto";
										...
										if ($_REQUEST["showonly_box"] == 1){
											...
											$tmpTDstr_new .= "<td id='td_cal_del" . $makctrl_unique . $inv["ID"] . "' style='background:#cee7c3;' width='150px;' align='center'><font size='4'><b>" . $b2b_fob . "</b></font>
											...
											$tmpTDstr_new .= "<td id='td_cal_del" . $makctrl_unique . $inv["ID"] . "' style='background:#cee7c3;' width='150px;' align='center'><font size='4'><b>" . $b2b_fob . "</b></font><br>

											<a href='#' onClick='calculate_delivery(" . $inv["ID"] . "," . $_REQUEST["ID"] . "," . $b2b_fob_main . "," . $_REQUEST["autoloadflg"] ."); return false;' >Calculate Delivery (FTL)</a>";



											$tmpTDstr_new .= '<input type="hidden" id="sales_price'.$inv["ID"].'" value="0"> <input type="hidden" id="ship_final'.$inv["ID"].'" value="0"></td>';
										...
										$tmpTDstr_new .= "<td bgColorrepl width='10px;' align='center'><img alt='' src='https://ucbloops.com/loops/matching_tool_icons/Icon_status.png' style='width:13px;height:13px;'/></td>";
										...
										$tmpTDstr_new .= "<td bgColorrepl width='10px;'><img alt='' src='https://ucbloops.com/loops/matching_tool_icons/Icon_truck.png' style='width:15px;height:15px;'/></td>";
										...
										$tmpTDstr_new .= "<td bgColorrepl width='10px;' align='center' ><img alt='' src='https://ucbloops.com/loops/matching_tool_icons/Icon_Location.png' style='width:10px;height:15px;'/></td>";
										...
										$tmpTDstr_new .= "<td bgColorrepl width='10px;'><img alt='' src='https://ucbloops.com/loops/matching_tool_icons/can_customer_pickup.png' style='width:15px;height:15px;'/></td>";
										...
										$tmpTDstr_new .= "<input type='hidden' id='av_tool_noofload_val' name='av_tool_noofload_val' value='$no_of_loads' ></tr>";
										...
										$tmpTDstr_new .= "<td bgColorrepl width='10px;'><img alt='' src='https://ucbloops.com/loops/matching_tool_icons/Icon_Dimensions.png' style='width:15px;height:15px;'/></td>";
										...
										$tmpTDstr_new .= "<td bgColorrepl width='10px;'><img alt='' src='https://ucbloops.com/loops/matching_tool_icons/Icon_supplier_owner.png' style='width:15px;height:15px;'/></td>";
										...
										$tmpTDstr_new .= "<td bgColorrepl width='10px;' align='center'><img alt='' src='https://ucbloops.com/loops/matching_tool_icons/Icon_notes.png' style='width:13px;height:13px;'/></td>";
										...
										if ($_REQUEST["showonly_box"] == 1){
											...
											if ($parent_loop_box_id > 0){
												...
												if (isset($_REQUEST["orgboxid"])){

													$orgboxid = $_REQUEST["orgboxid"];
												...
												$tmpTDstr_new .= "<td width='150px;' align='center' style='background:#99c796;' ><a href='javascript:void(0)' id='addtocartatag" . $inv["loops_id"] . "' onclick='display_child_box(".$_REQUEST["ID"] . "," . $inv["I"] . "," . $inv["loops_id"] ."," . $_REQUEST["gbox"] . "," . $orgboxid .")'><font size='4'><b>Add to Cart</b></font></a>"; 



												$tmpTDstr_new .= "<div style='display:none;' id='addtocart_parentdiv" . $inv["loops_id"] . "' ><table><tr><td style='background:#98bcdf;'>Add to Cart</td><td style='background:#98bcdf;'>Box</td></tr>";



												$qry_vendor1 = "Select loop_boxes.bdescription, b2b_id from loop_boxes where id = '". $parent_loop_box_id . "'";

												db();

												$dt_res_so1 = db_query($qry_vendor1);

												while ($so_row_main1 = array_shift($dt_res_so1)) {

													$parent_loop_box_desc = $so_row_main1["bdescription"];

													$parent_b2b_id = $so_row_main1["b2b_id"];



													$tmpTDstr_new .= "<tr><td width='150px;' align='center' style='background:#d7e3ee;' >

													<a href='javascript:void(0)' onclick='addgaylord(".$_REQUEST["ID"] . "," . $so_row_main1["b2b_id"] . ")'><font size='4'><b>Add to Cart</b></font></a></td>

													<td style='background:#d7e3ee;'>(Parent) " . $parent_loop_box_desc . " (ID " . $parent_b2b_id . ")</td>
												...
												$qry_vendor1 = "Select loop_box_id, loop_boxes.b2b_id, loop_boxes.bdescription from loop_boxes_parent_child inner join loop_boxes on loop_boxes.id = loop_boxes_parent_child.loop_box_id

												where parent_box_id = '". $inv["loops_id"] . "'";
												...
												while ($so_row_main1 = array_shift($dt_res_so1)) {
													...
													<td style='background:#d7e3ee;'>(Child) " . $so_row_main1["bdescription"] . " (ID " . $so_row_main1["b2b_id"] . ")</td>

													</tr>";

												}

												$tmpTDstr_new .= "</table></div>";
												...
												$tmpTDstr_new .= "<td width='150px;' align='center' style='background:#99c796;' ><a href='javascript:void(0)' onclick='addgaylord(".$_REQUEST["ID"] . "," . $inv["I"] . ")'><font size='4'><b>Add to Cart</b></font></a></td>";
										...
										$tmpTDstr_new .= "<tr><td colspan='11' style='line-height: 1px;'>&nbsp;</td></tr>";
								...
								$MGArray_new[] = array('arrorder' => $mileage, 'arrdet' => $tmpTDstr_new, 'sub_type' => $box_sub_type, 'territory' => $territory, 'box_urgent' => $inv["box_urgent"], 'available_load' => $no_of_loads, 'total_load' => $total_no_of_loads);
				...
				if ($_REQUEST["showonly_box"] == 1){
					...
					if ($_REQUEST["load_all"] == 1){
						...
						if ($_REQUEST["display-allrec"] == 2 ) {
							...
							$dt_view_qry = "SELECT * from tmp_inventory_list_set2 where wid <> 238 and actual > 0 and (type_ofbox = 'GaylordUCB' or type_ofbox = 'Gaylord') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2 or b2b_status=2.8) order by warehouse, type_ofbox, Description";
					...
					while ($dt_view_row = array_shift($dt_view_res)) {
						...
						$system_description = "";  $quantity_per_pallet = "";  $quantity = ""; 
						...
						if (($dt_view_row["actual"] != 0) OR ($dt_view_row["actual"] - $sales_order_qty !=0 )){
...

							{
								...
								if ($to_show_rec1 == "y"){
									...
									$no_of_loads = 0; $shipsinweek = 0; $to_show_rec = "";
									...
									if ($to_show_rec == "y"){	
										...
										if (isset($_REQUEST["display_view"]) && $_REQUEST["display_view"]=="1") {
											...
											<a target='_blank' href='https://ucbloops.com/b2b/?id=" . urlencode(encrypt_password($dt_view_row["trans_id"])) . "'>

											<img alt='' src='https://ucbloops.com/loops/boxpics_thumbnail/" . $bpic_1 . "' width='140px' height='140px;' ";
											...
											<a href='#' onClick='calculate_delivery(" . $b2bid_tmp . "," . $_REQUEST["ID"] . "," . $b2b_fob_main . ");  return false;' >Calculate Delivery (FTL)</a></td>";
											...
											$tmpTDstr_new .= "<td bgColorrepl width='10px;'><img alt='' src='https://ucbloops.com/loops/matching_tool_icons/Icon_status.png' style='width:15px;height:15px;'/></td>";
											...
											$tmpTDstr_new .= "<td bgColorrepl width='10px;'><img alt='' src='https://ucbloops.com/loops/matching_tool_icons/Icon_Location.png' style='width:15px;height:15px;'/></td>";
											...
											$tmpTDstr_new .= "<td bgColorrepl width='10px;'><img alt='' src='https://ucbloops.com/loops/matching_tool_icons/Icon_notes.png' style='width:15px;height:15px;'/></td>";
											...
											if (str_replace(",", "", $boxes_per_trailer_tmp) <> 0){

												$expected_loads_per_mo_for_3m = round(str_replace(",", "", $qty_avail_3month1)/str_replace(",", "", $boxes_per_trailer_tmp),2);
										...
										$MGArray[] = array('arrorder' => $mileage, 'arrdet' => $tmpTDstr_new, 'box_urgent' => $inv["box_urgent"]);
		...
		//echo "<pre>"; print_r($newLocArr); echo "</pre>";
				...
				foreach ($newLocArr as $newLocArrKey => $newLocArrValue) {
					...
					if(tep_db_num_rows($newLocArrValue) > 1 ){
						...
						for($i = 0; $i < tep_db_num_rows($newLocArrValue); $i++ ){
							...
							$qty_avail_3month = number_format($qty_avail_3month + $newLocArrValue[$i]['qty_avail_3month'], 0);
							...
							//if($newLocArrValue[$i]['expected_loads_per_mo'] > 0 ){ 
								...
								$arrShip[$newLocArrValue[$i]['I']]  = str_replace( ',', '', $newLocArrValue[$i]['qty'])/str_replace( ',', '', $newLocArrValue[$i]['boxes_per_trailer']);
					...
					if(tep_db_num_rows($newLocArrValue) > 1 ){
						...
						foreach ($arrENL as $arrENLKey => $arrENLValue) {
							...
							if($arrENLTemp[1] == 'Weeks' || $arrENLTemp[1] == 'Week' ){
								...
								$tempENLDays[$arrENLKey] = $arrENLTemp[0] * 7;
					...
					if($_REQUEST["sort_g_location"]=="2"){
						...
						$tmpTDstr2 =  $tmpTDstr2 . "<td><a name='location_details_show' id='loc_btn_".$vendor_b2b_rescue_id."' onClick='show_loc_dtls(".$vendor_b2b_rescue_id.",".tep_db_num_rows($newLocArrValue).",".$_REQUEST["sort_g_location"]." )' class='ex_col_btn' >" . $shipFrom ."</a></td>";
			...
			if(isset($_REQUEST["sort_g_lead"]) && $_REQUEST["sort_g_lead"] > 0){

				if($_REQUEST["sort_g_lead"] == 1){

					foreach ($MGArray_new as $MGArraytmp) {

						$MGArraysort[] = $MGArraytmp['available_load'];

					}

					array_multisort($MGArraysort, SORT_DESC, SORT_NUMERIC, $MGArray_new);

					

				}elseif($_REQUEST["sort_g_lead"] == 2){
					...
					array_multisort($MGArraysort, SORT_ASC, SORT_NUMERIC, $MGArray_new);

					

				}elseif($_REQUEST["sort_g_lead"] == 3){

					foreach ($MGArray_new as $MGArraytmp) {

						$MGArraysort[] = $MGArraytmp['total_load'];
